...
//&*&*&*QY_20110712, fix open ttyUSBx problem when system-resume 
static struct wake_lock urb_lock; 
...
//&*&*&*QY_20110712, fix open ttyUSBx problem when system-resume  
   if(!attach_flag) //Simon
   {
   	printk("[sierra module not attached]\n");
      return err;
   }  

   if(!wake_lock_active(&urb_lock))
   {
      wake_lock(&urb_lock); //simon
   }
//&*&*&*QY_20110712 : End
...
static void sierra_close(struct usb_serial_port *port)
{
	...
	if (serial->dev) {
		...
		if(attach_flag) //Simon
		{
			printk("[call sierra_stop_rx_urbs]\n");
...
static int sierra_open(struct tty_struct *tty, struct usb_serial_port *port)
{
	...
	if(!attach_flag)
	{
		printk("[sierra_open attach_flag=0]\n");
	   return -1; //Simon
	}
	//&*&*&*QY_20110712: End
...
static int sierra_resume(struct usb_serial *serial)
{
	...
	for (i = 0; i < serial->num_ports; i++) {
		...
		if (portdata->opened) {
			...
			{
				ec++;
				printk("[sierra_submit_rx_urbs error]\n");
			}
		}
	} 
...
static int __init sierra_init(void)
{
	...
	printk(KERN_INFO KBUILD_MODNAME ": " DRIVER_VERSION ":"
	       DRIVER_DESC "\n");
//&*&*&*QY_20110712, fix open ttyUSBx problem when system-resume 	       
   wake_lock_init(&urb_lock, WAKE_LOCK_SUSPEND, "sierra_urb_lock");
//&*&*&*QY_20110712 End   
	return 0;
...
static void __exit sierra_exit(void)
{
	...
	usb_serial_deregister(&sierra_device);
//&*&*&*QY_20110712, fix open ttyUSBx problem when system-resume 	
	wake_lock_destroy(&urb_lock);
	printk("sierra_urb_lock unlock\n");
//&*&*&*QY_20110712 end
}
