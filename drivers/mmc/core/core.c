...
#include <linux/mmc/card_socket.h> //&*&*&*SJ_20110628
...
/*		voltage = regulator_get_voltage(supply);
		if (voltage < 0)
			...
			result = 0;
*/
/*LH_SWRD_CL1_andy@20110701 don't control the power when plugin&plugout card-->*/
		if (result == 0 && !enabled)
...
u32 mmc_select_voltage(struct mmc_host *host, u32 ocr)
{
	int bit;
        ...
        pr_debug("%s: ocr: 0x%x, ocr_avail: 0x%08x\n", __FUNCTION__, ocr, host->ocr_avail);
	...
	if (bit) {
		...
		mmc_set_ios(host);
                ...
                printk("%s: host doesn't support card's voltages\n", mmc_hostname(host));
		ocr = 0;
	}
...
//&*&*&*SJ1_20110721, polling mmc1 card remove.
		...
		if (1 == host->index) {
			if (g_mmc1_poll_cnt > 0) {
				printk("start mmc1 poll :%d\n", g_mmc1_poll_cnt);
				--g_mmc1_poll_cnt;
				host->caps |= MMC_CAP_NEEDS_POLL;
			} else {
				if (host->caps & MMC_CAP_NEEDS_POLL) {
					g_mmc1_poll_cnt = 0;
					printk("stop mmc1 poll\n");	
					host->caps &= ~MMC_CAP_NEEDS_POLL;
//&*&*&*SJ1_20110812, add force card remove when polling mmc1 card remove time-out. 
					printk("force card remove\n");
					mmc_bus_get(host);
					...
					mmc_bus_put(host);
//&*&*&*SJ2_20110812, add force card remove when polling mmc1 card remove time-out.
				}
			}
		}
#endif		
